...
const STAMINA_DRAIN = 25; // per second while sprinting + moving
const STAMINA_REGEN = 15; // per second while not sprinting / not moving
...
function GameLogic({
  ...
  useFrame((_, delta) => {
    ...
    if (inCarRef.current.value) {
      ...
      const wantSprint = (desktopSprint || mobileSprint) && player.stamina > 0;
      const isSprinting = wantSprint && moving;
    ...
    // Zone damage only in offline mode
    if (
      !isOnline &&
      Math.sqrt(player.x * player.x + player.z * player.z) > zone.radius
    )
      player.health -= dt;
    ...
    // ── Game over checks (offline only) ──────────────────────────────────
    if (!isOnline && player.health <= 0 && gameActiveRef.current) {
      gameActiveRef.current = false;
      onDead(player.kills);
    }
    // In online mode, keep health at full (invincible)
    if (isOnline) {
      player.health = 100;
      player.armor = 50;
    }
